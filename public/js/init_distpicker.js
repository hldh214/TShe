function init_distpicker() {
    $('.province').each(function (_, value) {
        ...
        $(value).text($(value).distpicker('getDistricts')[province_no]);
    });

    $('.city').each(function (_, value) {
        let city_no = $(value).data('no');
        $(value).text($(value).distpicker('getDistricts', $(value).prev().data('no'))[city_no]);
    });

    $('.district').each(function (_, value) {
        let district_no = $(value).data('no');
        $(value).text($(value).distpicker('getDistricts', $(value).prev().data('no'))[district_no]);
    });
}
